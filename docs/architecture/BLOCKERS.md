...
| BLK-009 | CAC, ROAS, and LTV:CAC attribution/definition | Only the affected Conditional marketing metrics | `Marketing_Spend` supplies spend only; Shopify/Klaviyo attribution is limited to previously verified capabilities | Approve each numerator, denominator, blended-versus-attributed scope, and verified acquisition/revenue source before activating the affected metric | Open; does not block unrelated Marketing Intelligence |
| BLK-010 | Live Google production-credential verification | Backend Stage production gate | B4.5 focused non-live adapter, parser, isolation, and read-only verification passed | Supply an approved Google service account through server-side secret management, share only the four fixed allowlisted files as Viewer, and run the live read-only smoke verification | Deferred; non-blocking until Backend Stage gate |
| BLK-011 | Remaining B5 metric business definitions and thresholds | Only the 39 affected metrics | Definitive B5 metric-status record identifies revenue, order, AOV, refund, identity, channel, margin, inventory, finance, attribution, and alert rule gaps individually | Approve only the required definition or threshold before activating each affected metric; do not infer defaults | Open; does not block independent metrics |

## Non-blocking future input

Shopify, Klaviyo, and Google credentials are not required for focused non-live adapter verification under the approved Deferred Live Credential Verification rule. Request least-privilege read-only credentials only for the applicable live verification and receive them through approved server-side secret management, never committed files or chat.
