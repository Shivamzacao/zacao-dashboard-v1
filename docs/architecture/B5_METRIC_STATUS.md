...
Amendment (2026-08-09, Operations Intelligence activation): DEC-019 moved `operations.shipped_delivered` from `SOURCE_LIMITED` to `CERTIFIABLE` — DEC-016 approved passing the provider shipped/delivered aggregates through with carrier coverage disclosed, and suppressing the value was stricter than that decision. The same amendment adds one new registry entry, `operations.fulfillment_trend` (`CERTIFIABLE`, provider `orders_fulfilled` per reporting period), and adds `Operations Intelligence` to the sections of `products.units_sold` and `products.units_velocity` without altering their status or calculation. DEC-018 narrows the `inventory.shopify_current` headline to the provider `available` state; overlapping states are no longer summed. Active-status counts on the line above record the original B5 gate and are not restated here; `src/domain/metrics/catalog.ts` remains the runtime authority.
